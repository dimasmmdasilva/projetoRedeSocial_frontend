...
function useLayoutItem(options) {
    ...
    const { layoutItemStyles, layoutItemScrimStyles } = layout.register(vm, {
        ...options,
        active: computed(() =>
            isKeptAlive.value ? false : options.active.value
        ),
        id,
    })
...
function createLayout(props) {
    ...
    const computedOverlaps = computed(() => {
        ...
        for (const overlap of overlaps.filter((item) => item.includes(':'))) {
            ...
            if (
                !registered.value.includes(top) ||
                !registered.value.includes(bottom)
            )
                continue
    ...
    const layers = computed(() => {
        ...
        for (const p of uniquePriorities) {
            const items2 = registered.value.filter((id) => {
                var _a
                return (
                    ((_a = priorities.get(id)) == null ? void 0 : _a.value) ===
                    p
                )
            })
            layout.push(...items2)
    ...
    const transitionsEnabled = computed(() => {
        return !Array.from(disabledTransitions.values()).some(
            (ref2) => ref2.value
        )
    })
    ...
    provide(VuetifyLayoutKey, {
        register: (vm, _ref2) => {
            ...
            const instances = findChildrenWithProvide(
                ...
                rootVm == null ? void 0 : rootVm.vnode
            )
            ...
            const layoutItemStyles = computed(() => {
                ...
                if (!item)
                    throw new Error(
                        `[Vuetify] Could not find layout item "${id}"`
                    )
                ...
                return {
                    ...
                    bottom:
                        position.value !== 'top' ? `${item.bottom}px` : void 0,
                    width: !isHorizontal
...
var useIcon = (props) => {
    ...
    const iconData = computed(() => {
        ...
        const iconSetName = Object.keys(icons.sets).find(
            (setName) =>
                typeof icon === 'string' && icon.startsWith(`${setName}:`)
        )
...
async function scrollTo(_target, _options, horizontal, goTo) {
    ...
    const options = mergeDeep(
        (goTo == null ? void 0 : goTo.options) ?? genDefaults2(),
        _options
    )
    ...
    return new Promise((resolve) =>
        requestAnimationFrame(function step(currentTime) {
            ...
            if (
                progress >= 1 &&
                Math.abs(location - container[property]) < 10
            ) {
...
function getWeekArray(date2, locale, firstDayOfWeek) {
    ...
    const first =
        firstDayOfWeek ?? firstDay[locale.slice(-2).toUpperCase()] ?? 0
...
function isSameDay(date2, comparing) {
    return (
        date2.getDate() === comparing.getDate() &&
        date2.getMonth() === comparing.getMonth() &&
        date2.getFullYear() === comparing.getFullYear()
    )
}
...
function getDiff(date2, comparing, unit) {
    ...
    switch (unit) {
        ...
        case 'weeks':
            return Math.floor(
                (d.getTime() - c.getTime()) / (1e3 * 60 * 60 * 24 * 7)
            )
        case 'days':
            return Math.floor(
                (d.getTime() - c.getTime()) / (1e3 * 60 * 60 * 24)
            )
        case 'hours':
...
var VuetifyDateAdapter = class {
    ...
    getWeekArray(date2, firstDayOfWeek) {
        return getWeekArray(
            date2,
            this.locale,
            firstDayOfWeek ? Number(firstDayOfWeek) : void 0
        )
    }
...
function createInstance(options, locale) {
    const instance = reactive(
        typeof options.adapter === 'function'
            ? new options.adapter({
                  locale:
                      options.locale[locale.current.value] ??
...
//# sourceMappingURL=chunk-PQ42HMFX.js.map
